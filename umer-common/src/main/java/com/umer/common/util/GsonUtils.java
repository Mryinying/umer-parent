...
//                .setPrettyPrinting() //自动格式化换行
    ...
    /**
     * 根据对象返回json  true过滤空值字段  false不过滤空值字段
     */
    public static String toJsonString(Object object, boolean filter) {
        if (filter){
    ...
    static class LocalTimeAdapter implements JsonSerializer<LocalTime> {
        ...
        public JsonElement serialize(LocalTime localDateTime, Type type, JsonSerializationContext jsonSerializationContext) {
            return new JsonPrimitive(localDateTime.format(DateTimeFormatter.ofPattern("HH:mm:ss")));
        }
    }
